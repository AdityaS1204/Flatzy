    ...
    <div className="min-h-screen font-sans bg-[#FADA7A]">
      ...
      <div className="flex justify-center w-full gap-4 mt-8">
        <div className='flex justify-between w-6/12'>
          <Link to="/listings"><button className="bg-black text-white ring-2 ring-white px-6 py-3 rounded-full hover:scale-105 hover:cursor-pointer duration-300 transition">
            Find Your Stay
          </button></Link>
          <button className="border  text-black ring-2 ring-black px-6 py-3 rounded-full hover:bg-gray-100 transition duration-300" onClick={()=> whatsappRedirect(listmsg)}>
            List Your Property
          </button>
        </div>
      </div>
      ...
      viewport={{once:true,amount:0.3}}
       className='fixed bottom-10 right-10 bg-green-600 z-30 text-black p-4 rounded-full shadow-lg' onClick={()=>whatsappRedirect(chatmsg)}>Chat On Whatsapp</motion.div>
    </div>
  )
}
